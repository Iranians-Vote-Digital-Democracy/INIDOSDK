...
public:
    ...
    void ApplyPatch(void* address, void* data, uintptr_t size) {
        DWORD oldProtect;
        if (!VirtualProtect(address, size, PAGE_EXECUTE_READWRITE, &oldProtect))
        {
            printf("Failed to change memory protection | error = %#.8x\n", GetLastError());
            return;
        }
        ...
        {
            printf("Failed to restore memory protection | error = %#.8x\n", GetLastError());
            return;
        }
    }

    void ApplyNops(void* address, uintptr_t size) {
        DWORD oldProtect;
        if (!VirtualProtect(address, size, PAGE_EXECUTE_READWRITE, &oldProtect))
        {
            printf("Failed to change memory protection | error = %#.8x\n", GetLastError());
    ...
    void DisableIncompatibleAPDUCommands()
    {
        {
            printf("[+] Attempting to disable ATR check\n");
            unsigned char patchData[] = { 0xE9, 0xEF, 0x00, 0x00, 0x00, 0x90 };
            ApplyPatch((void*)0x18002F8F6, patchData, sizeof(patchData));
        }
        {
            printf("[+] Attempting to disable call to Mav4::Read_SOD2_Antiyes_Pubkey\n");
            unsigned char patchData[] = { 0x48, 0xC7, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x90, 0x90, 0x90 };
            ApplyPatch((void*)0x180029FA8, patchData, sizeof(patchData));
        }
        {
            printf("[+] Attempting to disable call to Mav4::Genuine_Replica\n");
            unsigned char patchData[] = { 0x48, 0xC7, 0xC0, 0x00, 0x00, 0x00, 0x00, 0x90, 0x90, 0x90 };
            ApplyPatch((void*)0x18002A210, patchData, sizeof(patchData));
        }

        {
            printf("[+] Attempting to disable call to dk_GetCardPinStatus_A\n"); // in dk_pinEnterSomething_A
            unsigned char patchData[] = { 0xE9, 0x4E, 0x01, 0x00, 0x00, 0x90, 0x90, 0x90 };
            ApplyPatch((void*)0x1802FEB29, patchData, sizeof(patchData));
        } 

        // NOP two function calls:
        //  dk_sendSoapWebRequest_0((__int64 *)rcx0, (__int64)&v13, &v17);
        //  dk_ParseJsonAuthenticationResult((char*)&v17, (_DWORD*)arg38);
        ApplyNops((void*)0x1803040EF, 21);
    }
    ...
    bool SetDeviceInfo() {
        ...
        CommandParameterStr<10244> cardReaderName("ACS ACR122 0");
    ...
    bool SetUIOptions() {
        ...
        if (!showPinInputUI) {
            printf("\n[!!] Warning ~~ showPinInputUI is set to false !!\n\n");
        }
  ...
  /*         
           printf("[+] About to call GetMocFinger1And2Index\n"); 
            ...
            // -------------------------------------------------------
            // let's unblock our PIN if it has been blocked
            // -------------------------------------------------------
           /* NIDUnblockPin unblockPin;
            try
            {
                printf("[+] About to call NIDUnblockPIN command\n");
                printf("[+] press any key to continue  NIDUnblockPIN command\n");
                getchar();
                std::string json = unblockPin.NIDUnblockPIN1((char*)m_library);
                std::cout << "NIDUnblockPin successful. JSON result: " << json << std::endl;
            }
            catch (const std::runtime_error& e)
            {
                std::cerr << "NIDUnblockPIN failed: " << e.what() << std::endl;
            }
            catch (...)
            {
                std::cerr << "NIDUnblockPIN: An unexpected error occurred." << std::endl;
            }*/
...
int main()
{
    DK_MDAS dk_mdas;
    if (dk_mdas.load()) {
        dk_mdas.DisableIncompatibleAPDUCommands();
        dk_mdas.run();
    }
    return 0;
}
