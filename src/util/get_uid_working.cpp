...
SCARDCONTEXT initializeContext() {
  ...
  if (status != SCARD_S_SUCCESS) {
    std::cerr << "Failed to establish context. Error: " << std::hex << status
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
std::vector<std::string> listReaders(SCARDCONTEXT context) {
  ...
  LONG status = SCardListReadersA(context, nullptr,
                                  reinterpret_cast<LPSTR>(&readersMultiString),
                                  &readersLen);

  if (status != SCARD_S_SUCCESS) {
    std::cerr << "Failed to list readers. Error: " << std::hex << status
              << std::endl;
    exit(EXIT_FAILURE);
  }
...
SCARDHANDLE connectToReader(SCARDCONTEXT context,
  ...
  if (status != SCARD_S_SUCCESS) {
    std::cerr << "Failed to connect to reader: " << readerName
...
std::vector<uint8_t> getCardUID(SCARDHANDLE cardHandle) {
  ...
  LONG status = SCardTransmit(cardHandle, SCARD_PCI_T1, command,
                              sizeof(command), nullptr, response, &responseLen);

  if (status != SCARD_S_SUCCESS) {
    std::cerr << "Failed to transmit command. Error: " << std::hex << status
              << std::endl;
    exit(EXIT_FAILURE);
  }

  // Validate response length and success status
  if (responseLen < 2 || response[responseLen - 2] != SW1_SUCCESS ||
      response[responseLen - 1] != SW2_SUCCESS) {
    std::cerr << "Invalid response from card. SW1: " << std::hex
              << response[responseLen - 2] << " SW2: " << std::hex
              << response[responseLen - 1] << std::endl;
    exit(EXIT_FAILURE);
  }
