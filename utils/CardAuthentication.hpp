...
private:
    ...
    // 5) The real “setSpSignature(...)” logic
    int64_t setSpSignature(char* libraryPtr);

    // ------------------------------------------------------------------------
    // Additional private helper methods (from your reversed code)
    // ------------------------------------------------------------------------

    // sub_1400B0FB0(...) => generate random nonce, or do Mersenne Twister, etc.
    // We'll store a random hex string or 64-bit in *a2.
    uint64_t generateNonce();

    // A minimal ISO8601 function. If you want exactly the reversed code,
    // replicate `_time64`, `_gmtime64_s`, `_ftime64_s`, etc.
 ...
 */
    ...
    /**
     * A more complete version of the reversed "dk_SetSpSignatureData(...)"
     * that calls HTTP, base64, etc.
     * @param libraryPtr   The library handle used in pfn_* calls
     * @param inputData    Some input text if needed by HTTP
     * @return             The new instance paramId=50 if success, or 0 if fail
     */
